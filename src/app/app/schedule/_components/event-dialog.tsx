...
export function EventDialog({
  ...
  useEffect(() => {
    ...
    if (dateToUse) {
      const defaultInstructorId = event?.instructorId
        || (role === 'schoolInstructor' ? user?.uid : null)
        || (canManageInstructorSchedules ? instructors[0]?.uid : null)
        || user?.uid
        || null;
      let startTimeStr = '';
      if (event) {
  ...
  const onSubmit = async (data: EventFormData) => {
    ...
    try {
        ...
        const selectedInstructorId = data.instructorId || null;

        if (canManageInstructorSchedules && instructors.length > 0 && !selectedInstructorId) {
          form.setError('instructorId', {
            type: 'manual',
            message: 'Choose the instructor who will handle this schedule.',
          });
          return;
        }
