...
export function CurrentBill({
  ...
  useEffect(() => {
    if (activeBill) {
        ...
        setNotes('');
        setDiscount(0);
        setApplyTax(Boolean(tenant?.taxEnabledByDefault));
  ...
  const handleFinalize = async () => {
    ...
    const paymentData = {
      ...
      tax: finalTax,
      taxLabel: applyTax && finalTax > 0 ? taxLabel : undefined,
      taxRate: applyTax && finalTax > 0 ? taxRate : undefined,
      total: finalTotal,
  ...
  const handleUpdateBill = async () => {
    ...
    const billData: Omit<Payment, 'id'> = {
      ...
      discount,
      tax: taxAmount,
      taxLabel: applyTax && taxAmount > 0 ? taxLabel : undefined,
      taxRate: applyTax && taxAmount > 0 ? taxRate : undefined,
      total,
