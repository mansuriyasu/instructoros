...
import {
  getScanErrorMessage,
  prepareLicenseFileForAi,
  scanLicenseFile,
} from "@/lib/license-scan-client";
import {
  Select,
  SelectContent,
  SelectItem,
...
export function StudentDetailsDialog({
  ...
  const mergeCandidates = useMemo(() => {
    ...
    const normalizedLicense = (student.licenseNumber || "")
      .replace(/\W/g, "")
      .toLowerCase();
    ...
    return students
      ...
      .map((candidate) => {
        ...
        const candidateMobile = (candidate.mobileNumber || "").replace(
          /\D/g,
          "",
        );
        const candidateLicense = (candidate.licenseNumber || "")
          .replace(/\W/g, "")
          .toLowerCase();
        const score =
          (normalizedLicense && candidateLicense === normalizedLicense
            ? 4
            : 0) +
  ...
  const selectedMergeStudent = useMemo(
    () =>
      mergeCandidates.find((candidate) => candidate.id === mergeStudentId) ||
      null,
  ...
  const allAvailableTags = useMemo(() => {
    const tagsSet = new Set<string>(
      Array.isArray(tenant?.customerTags)
        ? tenant.customerTags
        : defaultCustomerTags,
    );
    (students || []).forEach((s) => {
  ...
  const handleMergeDuplicate = async () => {
    if (!selectedMergeStudent || !student) {
      toast({
        variant: "destructive",
        title: "Choose a duplicate contact first.",
      });
      return;
    }
    ...
    try {
      const mergedComments = [
        student.comments,
        selectedMergeStudent.comments
          ? `Merged from ${selectedMergeStudent.name}: ${selectedMergeStudent.comments}`
          : "",
      ]
        .filter(Boolean)
        .join("\n\n");
      ...
      await updateStudent({
        id: student.id,
        mobileNumber:
          student.mobileNumber || selectedMergeStudent.mobileNumber || "",
        address: student.address || selectedMergeStudent.address || "",
        birthdate: student.birthdate || selectedMergeStudent.birthdate || "",
        licenseNumber:
          student.licenseNumber || selectedMergeStudent.licenseNumber || "",
        licenseExpiry:
          student.licenseExpiry || selectedMergeStudent.licenseExpiry || "",
        licenseType: student.licenseType || selectedMergeStudent.licenseType,
        status:
          student.status === "deactivated"
            ? selectedMergeStudent.status
            : student.status,
        registrationDate: new Date(
          Math.min(
            new Date(student.registrationDate || new Date()).getTime(),
            new Date(
              selectedMergeStudent.registrationDate || new Date(),
            ).getTime(),
          ),
        ).toISOString(),
  ...
  const handleAddToBill = () => {
    if (student) {
      onOpenChange(false);
      router.push(`/app/payments?studentId=${student.id}`);
    }
  };
  ...
  const handleBillLessons = (lessonIds: string[]) => {
    ...
    onOpenChange(false);
    router.push(
      `/app/payments?studentId=${student.id}&eventIds=${encodeURIComponent(lessonIds.join(","))}`,
    );
  };

  const handleScheduleExam = () => {
    onOpenChange(false);
    router.push(`/app/schedule?examStudentId=${student.id}`);
  };
  ...
  const handleSaveToContacts = () => {
    ...
    toast({
      ...
      description: "Open the file to save to your contacts.",
    });
  };

  const handleProfileLicenseScan = async (
    ...
    toast({
      title: "Saving and scanning license...",
      description: `The license image is being saved to ${student.name}'s profile first.`,
    });
    ...
    try {
      ...
      try {
        ...
        toast({
          ...
          description:
            "The full license image was saved. Please edit the details manually or try scanning again.",
        });
        return;
      }
  ...
  return (
    <>
        ...
        >
            ...
            <div className="flex justify-between items-start relative z-10">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3 pt-1">
                    ...
                    <span className="text-white/50 text-xs sm:text-sm flex items-center gap-1 font-medium">
                      ...
                      Joined{" "}
                      {format(new Date(student.registrationDate), "MMM yyyy")}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                ...
                <Button
                  ...
                  className="bg-[#C9A84C] text-[#0D1B2A] hover:bg-[#F0D080] shadow-md font-semibold border-0 hidden sm:flex h-9 rounded-full px-4"
                >
                ...
                <Button
                  onClick={handleAddToBill}
                  size="icon"
                  className="bg-[#C9A84C] text-[#0D1B2A] hover:bg-[#F0D080] shadow-md border-0 sm:hidden h-9 w-9 rounded-full"
                >
                ...
                <DropdownMenu>
                  ...
                  >
                    ...
                    <DropdownMenuItem
                      ...
                      className="mt-1 rounded-md text-red-600 focus:bg-red-50 focus:text-red-600 dark:text-red-500 dark:focus:bg-red-950/50"
                    >
              ...
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-4">
                <div className="group">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleCopyToClipboard(student.licenseNumber, "License")
                      }
                ...
                <div className="flex gap-8 sm:justify-end sm:pr-4">
                  <div className="group">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleCopyToClipboard(student.licenseExpiry, "Expiry")
                        }
                  ...
                  <div>
                    ...
                    <Badge
                      variant="secondary"
                      className="text-sm sm:text-base font-bold bg-[#0D1B2A] text-[#F0D080] dark:bg-[#C9A84C] dark:text-[#0D1B2A] border-0 rounded-md px-3"
                    >
                      {student.licenseType}
                    </Badge>
                  </div>
                ...
                {student.licenseImageUrl && (
                  <div className="col-span-1 sm:col-span-2 flex flex-col gap-2 mt-2 sm:-mt-2 border-t border-border/40 pt-4 sm:border-t-0 sm:pt-0">
                    <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold sm:text-right">
                      Saved license image
                    </p>
                ...
                {!student.licenseImageUrl && (
                  <div className="col-span-1 sm:col-span-2 flex items-center gap-2 rounded-xl border border-dashed border-border/70 bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                    ...
                    No license image saved yet. Use Scan License from the menu
                    to attach one.
                    {isUploadingLicense && (
          ...
          <div className="p-6 sm:p-8 pt-6">
            <Tabs defaultValue="contact" className="w-full">
                ...
                <div className="relative z-10 rounded-2xl border border-border/50 bg-muted/30 p-4">
                  ...
                  >
                    ...
                    <Button
                      type="submit"
                      size="sm"
                      className="h-9 bg-[#C9A84C] text-[#0D1B2A] hover:bg-[#F0D080]"
                    >
                      Add
                    </Button>
                  </form>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {student.portalStatus === "active"
                          ? `Active${student.portalEmail ? ` · ${student.portalEmail}` : ""}`
                          : student.portalStatus === "revoked"
                            ? "Revoked"
                            : "Not activated"}
                      </p>
                    </div>
                    {student.portalStatus === "active" ? (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        disabled={isPortalAccessBusy}
                        onClick={() => void handlePortalAccess("revoke")}
                      >
                        Revoke access
                      </Button>
                    ) : student.portalUid ? (
                      <Button
                        ...
                        size="sm"
                        disabled={isPortalAccessBusy}
                        onClick={() => void handlePortalAccess("restore")}
                      >
                        Restore access
                      </Button>
                    ) : (
                      <span className="text-xs text-muted-foreground">
                        Student activates from the registration link.
                      </span>
                    )}
                  </div>
                </div>
              ...
              <TabsContent
                value="lessons"
                className="outline-none focus-visible:ring-0 animate-in fade-in duration-300"
              >
                        ...
                        <div key={entitlement.serviceId}>
                          <div className="flex items-center justify-between gap-2 text-xs">
                            ...
                            <span className="shrink-0 text-muted-foreground">
                              ...
                              {entitlement.booked > 0
                                ? ` · ${entitlement.booked} booked`
                                : ""}{" "}
                          ...
                          <Progress
                            ...
                            value={
                              entitlement.purchased > 0
                                ? ((entitlement.used + entitlement.booked) /
                ...
                <div className="border border-border/50 rounded-xl overflow-hidden bg-card shadow-sm">
                  <div className="flex flex-col gap-3 border-b bg-muted/20 p-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleBillLessons(
                          billableTakenLessons.map((lesson) => lesson.id),
                        )
                      }
                      ...
                      {studentLessons.length > 0 ? (
                          ...
                          return (
                            <div
                              ...
                              onClick={() => {
                                onOpenChange(false);
                                router.push(
                                  `/app/schedule?eventId=${lesson.id}`,
                                );
                              }}
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-2 group-hover:text-primary transition-colors">
                                    ...
                                    <p className="font-semibold text-sm">
                                      {format(lessonDate, "MMM dd, yyyy")} at{" "}
                                ...
                                <div className="flex flex-col items-end gap-1.5">
                                  <Badge
                                    className={cn(
                                      "shadow-none",
                                      getLessonStatusClass(lesson.lessonStatus),
                                    )}
                                  >
                                    {getLessonStatusLabel(lesson.lessonStatus)}
                                  </Badge>
                                  {isPackageCovered ? (
                                    <Badge className="bg-violet-600 text-white shadow-none hover:bg-violet-600">
                                      Package
                                    </Badge>
                                  ) : (
                                    <Badge
                        ...
                        <div className="flex min-h-[180px] flex-col items-center justify-center p-6 text-center text-muted-foreground">
                          ...
                          <p className="text-xs opacity-70 mt-1">
                            Scheduled lessons for this student will show here.
                          </p>
                        </div>
                      )}
                    </div>
              ...
              >
                <div className="rounded-xl border border-border/50 bg-card shadow-sm">
                  <div className="border-b bg-muted/20 p-4">
                    <p className="text-sm font-semibold">
                      Mock road-test history
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      Instructor estimates only, not official DriveTest scores.
                    </p>
                  </div>
                  <div className="divide-y">
                    {evaluations.length > 0 ? (
                      evaluations.map((evaluation) => (
                        ...
                        >
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">
                              {evaluation.minor_count} minor ·{" "}
                              {evaluation.major_count} major
                            </p>
                          </div>
                          <Badge
                            className={cn(
                      ...
                      <div className="p-8 text-center text-sm text-muted-foreground">
                        No mock evaluations recorded yet.
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="bg-card border border-border/50 rounded-xl p-5 min-h-[120px] shadow-sm relative overflow-hidden">
                    ...
                    {student.comments ? (
                      <div className="flex gap-4">
                        ...
                        <p className="text-sm sm:text-base whitespace-pre-wrap leading-relaxed text-foreground/90">
                          {student.comments}
                        </p>
                      </div>
                    ) : (
                    ...
                    <Table>
                      <TableHeader className="bg-muted/40 sticky top-0 z-10 backdrop-blur-md">
                        <TableRow className="hover:bg-transparent">
                          ...
                          <TableHead className="font-bold text-xs uppercase tracking-wider text-muted-foreground text-right h-10 pr-6">
                            Status
                          </TableHead>
                        </TableRow>
                      </TableHeader>
                            ...
                            >
                              <div className="flex flex-col items-center justify-center">
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={handleAddToBill}
                                  className="mt-1 h-auto p-0 text-[#C9A84C]"
                                >
      ...
      />
      <AlertDialog
        open={isAddressNavigationOpen}
        onOpenChange={setIsAddressNavigationOpen}
      >
      ...
      <Dialog open={isMergeOpen} onOpenChange={setIsMergeOpen}>
        <DialogContent className="max-w-md rounded-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium">
                Duplicate contact to merge and delete
              </p>
            ...
            {selectedMergeStudent && (
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                This will move payments and scheduled lessons from{" "}
                {selectedMergeStudent.name} to {student.name}, then delete the
                duplicate record.
              </div>
            )}
