...
import { ChevronDown, Clock3, Link as LinkIcon, Loader2, Save, X } from 'lucide-react';
    ...
    <div aria-label={`${studentName} availability`} className="relative z-10 rounded-2xl border border-border/50 bg-muted/30 p-4 mt-4">
      ...
      >
        <div className="flex min-w-0 items-center gap-2">
          <LinkIcon className="h-4 w-4 shrink-0 text-[#C9A84C]" />
          <div className="min-w-0">
            <p className="text-sm font-semibold text-foreground">Student Availability</p>
            <p className="truncate text-xs text-muted-foreground">
              {draftWindows.length > 0 ? `${draftWindows.length} saved time window${draftWindows.length === 1 ? '' : 's'}` : 'No availability configured'}
            </p>
          </div>
        </div>
      ...
      {expanded && (
        <>
          {!availability && draftWindows.length === 0 && (
            <p className="mb-2 mt-3 text-sm text-muted-foreground">No availability configured.</p>
          )}
