...
export function StudentTagSettings() {
  ...
  useEffect(() => {
    const configured = Array.isArray(tenant?.customerTags) ? tenant.customerTags.map(normalizeTag).filter(Boolean) : [];
    setTags(Array.from(new Set([...DEFAULT_TAGS, ...configured])));
  }, [tenant?.customerTags]);
