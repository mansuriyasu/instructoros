...
export function PaymentDetailsDialog({
  ...
  const generatePdf = async () => {
    ...
    doc.setProperties({
      title: `SparkOn Receipt - ${payment.studentName}`,
      subject: 'Payment Receipt',
      author: 'SparkOn Driving Academy',
    });
    ...
    const drawSparkonLogo = (x: number, y: number) => {
      if (logoDataUrl) {
      ...
      doc.text('SparkOn', x, y + 7);
      doc.setFontSize(6.5);
      doc.setCharSpace(1.8);
      doc.text('DRIVING ACADEMY', x, y + 14);
      doc.setCharSpace(0);
    };
    ...
    const summaryRows = [
      ...
      ...(payment.tax > 0 ? [{ label: 'Tax', value: formatCurrency(payment.tax) }] : []),
    ...
    doc.setFontSize(10);
    doc.setTextColor(...ink);
    doc.text('Thank you for choosing SparkOn Driving Academy.', margin, cursorY);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.setTextColor(...muted);
    ...
    for (let page = 1; page <= pageCount; page += 1) {
      ...
      doc.setFontSize(8);
      doc.setTextColor(...muted);
      doc.text('SparkOn Driving Academy | 438-926-2048 | www.sparkondrive.ca', margin, pageHeight - 10);
