    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-4 pb-24">
      ...
      <section className="grid gap-3 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-2xl border border-border/60 bg-card p-4 shadow-sm">
          ...
          {nextLesson ? (
            <Link
              href={`/schedule?eventId=${nextLesson.id}`}
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-4 shadow-sm">
          ...
          <div className="space-y-2">
            {todayLessons.slice(0, 3).map(lesson => (
              <Link
                key={lesson.id}
                href={`/schedule?eventId=${lesson.id}`}
      ...
      <section>
        ...
        <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 lg:grid-cols-5">
          <ActionTile href="/students" icon={Users} label="Students" tone="gold" />
          <ActionTile href="/schedule" icon={CalendarDays} label="Schedule" tone="blue" />
          <ActionTile href="/payments" icon={Wallet} label="POS" tone="green" />
          <ActionTile href="/payments/history" icon={History} label="History" tone="purple" />
          <ActionTile href="/expenses" icon={ReceiptText} label="Expenses" tone="teal" />
          <ActionTile href="/utility-tracker" icon={Activity} label="Utility" tone="red" />
          <ActionTile href="/settings?tab=import-export" icon={Upload} label="Import" tone="blue" />
          <NotificationsTile />
          <ActionTile href="/settings" icon={Settings} label="Settings" tone="slate" />
        </div>
      </section>
    </div>
  );
