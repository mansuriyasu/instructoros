...
export default function StudentIntakePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const createAccount = async () => {
    ...
    try {
      const accountUser = user || (await createUserWithEmailAndPassword(auth, fields.email.trim().toLowerCase(), password)).user;
    ...
    } catch (error) { setError(error instanceof Error ? error.message : 'Could not create your student account.'); }
