...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ evaluationId: string }> }) {
  try {
    ...
    const allowed = ['testType', 'date', 'area', 'instructor', 'items', 'minor_count', 'major_count', 'autofails', 'verdict', 'notes'];
    const changes = Object.fromEntries(allowed.filter(key => Object.prototype.hasOwnProperty.call(body, key)).map(key => [key, body[key]]));
