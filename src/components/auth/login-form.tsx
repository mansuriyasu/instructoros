...
function getAuthErrorMessage(
  error: unknown,
  attemptType: AuthAttemptType = "email",
) {
  ...
  if (
    /auth\/popup-closed-by-user|auth\/cancelled-popup-request/i.test(message)
  ) {
...
function recordFailedAuthAttempt(attemptId: string) {
  ...
  const isFreshWindow =
    current && now - current.firstAttemptAt <= AUTH_ATTEMPT_WINDOW_MS;
  const nextCount = isFreshWindow ? current.count + 1 : 1;
  const lockedUntil =
    nextCount >= AUTH_ATTEMPT_LIMIT ? now + AUTH_LOCK_MS : undefined;
...
function shouldCountFailedAttempt(error: unknown) {
  const message = error instanceof Error ? error.message : String(error);
  return /auth\/invalid-credential|auth\/wrong-password|auth\/user-not-found|auth\/too-many-requests/i.test(
    message,
  );
}
...
export function LoginForm() {
  ...
  const requestedMode =
    requestedModeParam === "invite" && inviteTenantId && inviteId
      ? "invite"
  ...
  const [inviteUseExistingAccount, setInviteUseExistingAccount] =
    useState(false);
  ...
  const hasUnconnectedSession = Boolean(
    session.user && !session.isSessionLoading && !session.role,
  );
  const canUseCurrentSessionForSignup =
    hasUnconnectedSession &&
    sessionEmail === normalizedEmail &&
    mode !== "login";
  const requiresName =
    mode === "school" || mode === "solo" || mode === "invite";
  const canSubmit =
    (normalizedEmail.includes("@") ||
      Boolean(canUseCurrentSessionForSignup && sessionEmail.includes("@"))) &&
  ...
  useEffect(() => {
    ...
    router.replace(
      session.isMainAdmin && !session.activeTenantId ? "/admin" : nextUrl,
    );
  }, [
    nextUrl,
    router,
  ...
  const activateFreeTrial = async (user: User, tenantId: string) => {
    ...
    if (!response.ok || data.status !== "trialing") {
      throw new Error(
        data.error ||
          "Workspace was created, but the free trial could not be activated.",
      );
    }
  };
  ...
  const getExistingWorkspace = async (user: User) => {
    ...
    const memberSnap = await getDoc(
      doc(firestore, "tenants", activeTenantId, "members", user.uid),
    );
  ...
  const acceptInvite = useCallback(
    async (user: User, emailOverride?: string) => {
      ...
      .catch((joinError: unknown) => {
        const message =
          joinError instanceof Error
            ? joinError.message
            : "Could not join this school invite.";
        setError(
    ...
    try {
      ...
      if (canUseCurrentSessionForSignup && session.user) {
        ...
        try {
          ...
          if (
            mode === "invite" &&
            /auth\/email-already-in-use/i.test(
            ...
            throw new Error(
              "This email already has an InstructorOS account. Choose “Use existing account” above and sign in with that account’s current password, or use Google if it is a Google account.",
            );
          }
          throw signupError;
        }
          ...
          await activateFreeTrial(user, tenantId).catch((trialError) => {
            setNotice(
              trialError instanceof Error
                ? trialError.message
                : "Workspace created. You can activate the trial from Billing.",
            );
          });
        }
      ...
      clearAuthAttempts(attemptId);
      router.replace(
        normalizedEmail === MAIN_ADMIN_EMAIL && mode === "login"
          ? "/admin"
          : nextUrl,
      );
    } catch (submitError) {
      const lockoutMessage = shouldCountFailedAttempt(submitError)
        ? recordFailedAuthAttempt(attemptId)
        : "";
    ...
    try {
      ...
      if (mode === "school" || mode === "solo") {
        ...
        if (existingWorkspace?.role) {
          setError(
            "This Google account is already connected to an InstructorOS workspace. Use Login with Google, or choose a different Google account.",
          );
          return;
        }

        const tenantId = await createTenantWorkspace(
          user,
          mode === "school" ? "school" : "solo",
        );
        if (activateTrialOnSignup) {
          await activateFreeTrial(user, tenantId).catch((trialError) => {
            setNotice(
      ...
      router.replace(
        googleEmail === MAIN_ADMIN_EMAIL && mode === "login"
          ? "/admin"
          : nextUrl,
      );
    } catch (googleError) {
  ...
  const signupOptions = modeOptions.filter(
    (option) => option.value !== "login",
  );
  const selectedMode =
          ...
          <CardContent className="space-y-5 p-5 sm:p-6">
            {hasUnconnectedSession && mode !== "invite" && (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                ...
                <p className="mt-1">
                  This account is not connected to a workspace yet. Logout first
                  if you want to create a new school or instructor account with
                  a different email.
                </p>
                <Button
                  type="button"
            ...
            {mode === "invite" && isJoiningInvite && (
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
                <p className="font-semibold">
                  Joining your school workspace...
                </p>
                <p className="mt-1">
                  Your signed-in account matches the invite. We are connecting
                  it now.
                </p>
              </div>
            )}
              ...
              <div className="space-y-3 rounded-2xl border border-[#F4C430]/50 bg-[#FFF8D8] px-4 py-3 text-sm">
                <div>
                  ...
                  <p className="mt-1 text-muted-foreground">
                    Enter your email and password below, or use Google.
                  </p>
                </div>
                <Button
                  type="button"
                  variant="outline"
                  className="h-10 w-full rounded-xl bg-white font-bold"
              ...
              <div className="rounded-2xl border bg-muted/30 px-4 py-3">
                ...
                {mode === "invite" && inviteUseExistingAccount && (
                  <p className="mt-3 text-sm text-muted-foreground">
                    Sign in with the password already connected to this email.
                    Your invite will be accepted after login.
                  </p>
                )}
              </div>
            )}
            ...
            {mode === "invite" && (!inviteTenantId || !inviteId) && (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                Open the invite link from your school admin to accept a school
                instructor account.
              </div>
            )}
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <label className="flex items-center gap-3 rounded-2xl border bg-muted/30 px-3 py-3 text-sm">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setKeepLoggedIn(Boolean(checked))
                  }
                />
              ...
              {(mode === "school" || mode === "solo") && (
                <label className="flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-3 text-sm text-emerald-950">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setActivateTrialOnSignup(Boolean(checked))
                    }
                    className="mt-0.5"
                  />
                  <span>
                    <span className="block font-bold">
                      Activate my 1 month free trial now
                    </span>
                    <span className="mt-1 block text-emerald-900/80">
                      No payment details are required. Add payment later if you
                      want to continue after the trial.
                    </span>
                  </span>
                </label>
              )}
              ...
              <Button
                ...
                disabled={!canSubmit || Boolean(authLockMessage)}
              >
                {isSubmitting && (
            ...
            {mode === "login" && (
              <div className="space-y-3 rounded-2xl border bg-muted/20 p-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Choose the account type that matches why you are here.
                  </p>
                </div>
                    ...
                    ({ value, label, description, icon: Icon }) => (
                      ...
                      >
                        ...
                        <span className="min-w-0">
                          ...
                          <span className="mt-0.5 block text-xs font-medium leading-tight text-muted-foreground">
                            {description}
                          </span>
                        </span>
                      </Button>
